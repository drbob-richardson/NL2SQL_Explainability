...
limitations. We compare against a faithful re-implementation head-to-head (\S\ref{sec:exp}, Table~\ref{tab:bagel}). \emph{Graph retrieval.} GraphRAG and HopRAG use graph structure for retrieval; HopRAG builds logical
...
\paragraph{Head-to-head with the actual BAGEL algorithm.} Our nearest competitor is BAGEL~\citep{bagel2026}. We
reimplement it faithfully (code unreleased): a query-specific \emph{zero-mean} GP over passage embeddings with an
RBF kernel whose lengthscale is fit \emph{per query} by marginal likelihood, LLM relevance scores as standardized
observed labels, the query embedding seeded as a maximum-relevance pseudo-observation, a top-$M$ dense warm-start,
UCB acquisition ($\beta{=}2$), and final ranking by posterior mean; we match its total judgment budget to ours.
Across $B{=}1$--$10$ the graph method beats faithful BAGEL on recall@$k$ by $+0.15$ to $+0.25$, significant at every
budget (Table~\ref{tab:bagel}), and the gap decomposes cleanly into our two design choices. \emph{(i)
Semantics-in-the-mean:} BAGEL's zero-mean GP carries no cross-query prior and is starved at low budget; giving
BAGEL's own kernel our calibrated prior mean (``$+$our prior'') recovers about half the gap. \emph{(ii)
Structure-in-the-covariance:} even with our prior \emph{and} BAGEL's marginal-likelihood-tuned RBF kernel and
query-seed, the graph covariance still wins by $+0.09$--$0.14$ recall@$k$ at every budget---BAGEL's kernel-tuning
machinery does not close the covariance gap. The conclusion is unchanged under a maximally-active warm/active split
(warm$=1$: $\Delta$ vs BAGEL $+0.24$, vs $+$prior $+0.13$ at $B{=}3,5$).
\begin{table}[t]\centering\small
\begin{tabular}{lccccc}
\toprule
recall@$k$, chained ($n{=}600$) & $B{=}1$ & $B{=}2$ & $B{=}3$ & $B{=}5$ & $B{=}10$ \\ \midrule
faithful BAGEL (zero-mean)      & $0.446$ & $0.479$ & $0.490$ & $0.491$ & $0.578$ \\
\quad $+$ our calibrated prior  & $0.583$ & $0.579$ & $0.626$ & $0.628$ & $0.613$ \\
graph-GP (ours)                 & $\mathbf{0.692}$ & $\mathbf{0.714}$ & $\mathbf{0.720}$ & $\mathbf{0.725}$ & $\mathbf{0.729}$ \\ \midrule
$\Delta$ ours $-$ BAGEL         & $+.246$ & $+.235$ & $+.229$ & $+.235$ & $+.151$ \\
$\Delta$ ours $-$ ($+$prior)    & $+.109$ & $+.135$ & $+.094$ & $+.098$ & $+.116$ \\
\bottomrule
\end{tabular}
\caption{Faithful BAGEL vs.\ ours at matched judgment budget. All $\Delta$ have 95\% CIs excluding $0$ (e.g.\ at
$B{=}1$, ours$-$BAGEL $+0.246\,[.222,.269]$; ours$-(+$prior$)$ $+0.109\,[.089,.129]$). Row 2 (BAGEL's kernel with
our prior mean) isolates the covariance: the graph still wins, so the advantage is not merely a better prior or an
untuned lengthscale.}\label{tab:bagel}
\end{table}
