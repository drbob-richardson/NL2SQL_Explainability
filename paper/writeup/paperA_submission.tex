...
significant low-budget win on standard multi-hop (recall $+0.039$, nDCG@10 $+0.012$, chain-completion $+0.068$ at
...
propagation then fires. Empirically this roughly \emph{doubles} the graph$-$cosine chain-completion margin (from
$+0.068$ to $+0.12$--$0.15$ at one judgment). The finding is a small but real methodological point and is, to our
...
($n{=}600$), the graph kernel beats the embedding kernel at low budget across all three metrics
(Table~\ref{tab:main}): at one judgment, recall $+0.039\,[.024,.054]$, nDCG@10 $+0.012\,[.003,.023]$,
chain-completion $+0.068\,[.040,.097]$; significant at $B{=}1,2$, converging by $B{=}3$. The graph kernel---not
``any GP''---drives it: graph-GP $>$ cosine-GP $>$ passive.
\begin{table}[t]\centering\small
\begin{tabular}{lccc}
\toprule
graph$-$cosine (chained, real judge) & recall@$k$ & nDCG@10 & completion \\ \midrule
$B=1$ & $+0.039\,[.024,.054]$ & $+0.012\,[.003,.023]$ & $+0.068\,[.040,.097]$ \\
$B=2$ & $+0.027\,[.009,.045]$ & $+0.013\,[.002,.024]$ & $+0.057\,[.028,.087]$ \\
$B=3$ & $+0.005\,[\text{-}.015,.025]$ & $+0.012\,[\text{-}.000,.025]$ & $+0.012\,[\text{-}.020,.043]$ \\
\bottomrule
\end{tabular}
\caption{Low-budget graph-kernel win; significant at $B\le2$, converging by $B=3$.}\label{tab:main}
\end{table}
...
($+0.033\,[.007,.061]$ at $B{=}1$, real judge; $+0.041$--$0.059$ oracle) and is neutral on comparison
($-0.008$ to $+0.017$, n.s.)---the boundary the theorem predicts, with the mechanism visible: comparison questions
have a strong prior (passive recall $\sim0.8$; both entities directly findable, no bridge to propagate along). The
oracle-graph ceiling is $+0.07$--$0.09$, and gold-connectivity tracks the per-regime gain.
...
per query whether to use the graph. It does not deploy, and the reason is instructive. The per-query graph
advantage is a \emph{small mean effect swamped by variance}: pooled mean $+0.023$, sd $0.198$ (signal-to-noise
$\approx0.12$). It is nearly unpredictable from gold-free features \emph{even under the oracle}: ridge $R^2=0.034$
(generic features), only $0.052$ with theory-motivated alignment/reachability features. Two mechanistic fixes we
tried failed: the graph does \emph{not} amplify judge errors (advantage correlates $-0.17$ with anchor
reliability), and confidence-gating the propagation to high-confidence labels \emph{hurts} ($-0.030$; the
``related'' grade-$1$ labels carry useful signal). The structural gain is thus a \emph{distributional} property---a
robust small average in the deep multi-hop regime (the alignment law)---not a per-query-predictable signal. Since
the graph is a safe default (neutral-to-helpful, never significantly hurts under a real judge), the correct policy
is simply \emph{always use the graph}; per-query adaptivity is neither feasible nor needed.
