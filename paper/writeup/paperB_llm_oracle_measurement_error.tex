...
audits); and \emph{tensor / three-view identifiability} of latent-variable models (Kruskal; Allman, Matias \&
Rhodes), which supplies our proof machinery. Our novelty is fourfold: (i) misclassification that is
\emph{differential} with respect to the target, driven by a latent role that refines relevance; (ii) roles that
are \emph{graph-dependent} rather than exchangeable; (iii) \emph{structural identification}---an HMM/tensor
argument in which corpus dependence replaces replication or gold; and (iv) evaluation on intrinsic calibration
and a downstream decision rather than on inter-rater agreement.
