...
structure to add. We verify the two load-bearing results in simulation, and lay out the near-aliasing rate and
Bayesian weak-identification theory that would make this a T\&M paper. The empirical failure that prompted this
...
\section{Near-aliasing rates and Bayesian weak identification (the T\&M core)}
The results above are qualitative (identified / not). T\&M wants \emph{rates and limits} in the near-aliased
regime, $\delta_E=d(O_a,O_b)\to0$ and $\delta_G=d(Q_{L^\star}(a),Q_{L^\star}(b))\to0$.
\begin{conjecture}[Phase boundary]\label{conj:rate}
For classifying the $h$-functional at a node from its radius-$L^\star$ neighbourhood, the minimax risk decays
like $\exp\!\big(-c\,n\,(\delta_E^2+\delta_G^2)\big)$ (Hellinger/Chernoff separation of the mixed signatures), so
consistent recovery holds iff $n(\delta_E^2+\delta_G^2)\to\infty$, with a non-vanishing weak-identification
regime when it stays bounded.
\end{conjecture}
\CONJ{Derive the exact separation of $Q_{L^\star}(a),Q_{L^\star}(b)$ in Hellinger as a function of $(T,O)$ and
$\delta_E$; convert to minimax classification risk. The clean target is a boundary in $n(\delta_E^2+\delta_G^2)$.}
...
\begin{enumerate}[leftmargin=*,itemsep=1pt]
  ...
  condition).
  \item \textbf{Prove the near-aliasing rate} (Conj.~\ref{conj:rate}): the phase boundary in
  $n(\delta_E^2+\delta_G^2)$. This is the T\&M centerpiece---new mathematics, not an application.
