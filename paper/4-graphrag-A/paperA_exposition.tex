...
should come from a graph on the corpus. The graph is a bad \emph{standalone} ranker (ranking by structure alone
buries the answer) but an excellent \emph{covariance}: judging one passage lifts our belief about its
graph-neighbours, and that is exactly how a buried bridge gets surfaced. The rest of this document makes each italicized word precise.
...
PageRank), mass would spread to high-degree hubs and \emph{bury} the bridge: ranking by structure alone drops recall to
$0.21$ (vs.\ $0.65$ for the semantic prior, \S\ref{sec:main}), so the graph is a poor \emph{standalone} ranker. But
as a \emph{covariance} the graph is not asked to rank anything; it is asked to say ``if this
...
\begin{equation}
\mu_i=m_i+c\,K_{ia},\qquad c:=\frac{y_a-m_a}{1+\sigma^2}>0.
\label{eq:mu-rankone}
\end{equation}
The bridge $b$ enters the top-$k$ (overtakes every distractor) if and only if
\begin{equation}
K_{ba}\ >\ \max_{d\in D}\Big[K_{da}+\tfrac{m_d-m_b}{c}\Big],
\qquad\text{i.e.}\qquad K_{ba}-K_{da}>\tfrac{m_d-m_b}{c}\ \ \forall d.
\label{eq:surface}
\end{equation}
A convenient \emph{sufficient} condition, which decouples the two maxima, is that the \emph{kernel differential}
clears a single burial threshold,
\begin{equation}
K_{ba}-\max_{d\in D}K_{da}\ >\ \tau:=\frac{(1+\sigma^2)\,(\max_{d}m_d-m_b)}{y_a-m_a}.
\label{eq:surface-suff}
\end{equation}
\end{lemma}
...
\eqref{eq:mu-rankone}. The bridge overtakes distractor $d$ iff $\mu_b>\mu_d$, i.e.\ $m_b+cK_{ba}>m_d+cK_{da}$, i.e.\
$K_{ba}-K_{da}>(m_d-m_b)/c$; requiring this for \emph{every} $d$ is the exact condition \eqref{eq:surface}. Note it
is a maximum \emph{of the sum} $K_{da}+(m_d-m_b)/c$ and does \emph{not} in general separate into $\max_d K_{da}$ and
$\max_d m_d$---the distractor with the largest prior need not be the one with the largest covariance. Bounding each
term by its own maximum gives the clean sufficient condition \eqref{eq:surface-suff}, since then
$K_{ba}-K_{da}\ge K_{ba}-\max_{d'}K_{d'a}>\tau\ge(m_d-m_b)/c$ for every $d$.
\end{proof}
...
kernels. Being buried in the \emph{mean} ($m_b<m_d$) says nothing directly about the covariance differential; but
the bridges that matter here are also \emph{embedding-isolated}---$K^E_{ba}\le K^E_{da}$ for some higher-prior
distractor $d$, since being embedding-far from the anchor is what makes a bridge hard---so the embedding
differential is nonpositive and the bridge \emph{never} surfaces through $K^E$. For the graph kernel the differential can be large and positive:
...
\item[\textup{(i)}] \textbf{One-hop covariance surrogate, exact.} For $\corr(I+\beta A)$---a linearization used
only for this exact combinatorial calculation, not as a fitted GP covariance (it need not be PSD for arbitrary
$\beta$)---with $0<\tau<\beta$, the bridge surfaces if and only if $A_{ba}=1$ and $A_{da}=0$ for all $d$. Hence
\begin{equation}
...
\begin{proof}
\emph{(i)} The one-hop surrogate is $I+\beta A$; its correlation form has unit diagonal and off-diagonal
$\corr(I+\beta A)_{ij}=\beta A_{ij}$. Apply the \emph{exact} surfacing condition \eqref{eq:surface}: the bridge
surfaces iff $\beta(A_{ba}-A_{da})>(m_d-m_b)/c$ for every $d$. If some distractor is anchor-connected ($A_{da}=1$)
it gets the same lift as the bridge ($\beta A_{da}=\beta=\beta A_{ba}$) while $m_d>m_b$, so the bridge cannot pass
it; surfacing therefore forces $A_{da}=0$ for all $d$. Conversely, if $A_{ba}=1$ and all $A_{da}=0$, then
$\beta(1-0)=\beta>\tau\ge(m_d-m_b)/c$ for every $d$ (using $0<\tau<\beta$), so the bridge clears them. Hence the
bridge surfaces iff $A_{ba}=1$ and $A_{da}=0\ \forall d$; the corrected lemma leaves the one-hop count intact. The
edge $\{b,a\}$ is within the chain block (probability $p$); each edge $\{d,a\}$ is off-block (probability $q$), and
there are $|D|$ of them, independent. Therefore
\[
...
The theorem is stated in terms of unobservable block probabilities $p,q$. We measure the alignment $p-q$
\emph{retrospectively}, on labeled evaluation data (computing it needs the gold labels), by the empirical
\emph{assortativity} of the graph with respect to the gold/distractor labels,
\begin{equation}
...
prior; passive ``verify the top $B$''; a free graph-smoothed prior (label propagation); the embedding-kernel GP;
a faithful reconstruction of BAGEL \citep{bagel2026}; and two ceilings (an oracle judge that never errs, and an
oracle gold-clique graph). Metrics are
...
This is the crux of the mean/covariance thesis, so it is worth pinning down with a direct ablation. Ranking by
graph structure \emph{alone}---PageRank or node degree, ignoring the semantic scores---\emph{buries} the answer:
recall collapses to $0.209$ (completion $0.038$), far below the semantic prior's $0.646$ (Table~\ref{tab:ablate}).
A diffusion flows to high-degree hubs, not to the low-degree bridge; the graph is a genuinely poor \emph{standalone}
ranker. Used as a \emph{covariance}, though, it is load-bearing even for \emph{free}: label-propagation / GMRF
smoothing of the calibrated prior, $(I+\lambda\mathcal L)^{-1}m$ with no judgments, lifts recall to $0.707$ and
completion to $0.488$, because the graph is chain-assortative---the same denoising a spatial statistician applies to
a noisy field on a lattice. That free graph-smoothing is a strong baseline, stronger than passive or the
embedding-kernel GP, and the honest question is whether spending active judgments beats it. It does, but only
\emph{from the second judgment}: graph-GP ties the free smoother at $B{=}2$ and pulls ahead to $+0.043$ completion
$[.002,.087]$ by $B{=}10$, while at a \emph{single} judgment the free smoother is marginally ahead ($-0.043$
completion). The reading is not that active retrieval is pointless---it wins once the budget exceeds one---but that
the graph's value lives in the \emph{covariance} (smoothing or propagation), never as a standalone ranker, and that
a free graph-smoothing baseline belongs in any honest comparison. This is a humbler thesis than ``the graph helps
only with judgments,'' and a more defensible one.
\begin{table}[h]\centering\small
\caption{Structure alone buries; as a covariance it ranks well and, actively, beats even a free graph-smoothing
baseline from $B{=}2$. Chained, $n{=}600$; recall@$k$ / completion.}\label{tab:ablate}
\begin{tabular}{lcc}
\toprule
...
cosine-GP ($B{=}1$) & $0.634$ & $0.323$ \\
graph-GP, ours ($B{=}1$) & $0.692$ & $0.445$ \\
...
code is unreleased we reconstruct it faithfully and, to be scrupulously fair, we \emph{control for two things at
once}: we can give BAGEL our calibrated prior mean (isolating the effect of the mean) and we keep its own
marginal-likelihood-tuned kernel (isolating the effect of the covariance). All methods score with the \emph{same}
hop-aware judge, so the comparison isolates the retrieval algorithm from judge quality. We match total judgment budgets and sweep
$B$ from $1$ to BAGEL's exact native configuration ($B=50$: $25$ warm-start $+$ $25$ active, half the $N=100$ pool).
For $B<50$ we split the budget as $\lceil B/2\rceil$ warm-start (top-cosine) $+\lfloor B/2\rfloor$ active judgments,
the query pseudo-observation being free; the head-to-head is unchanged under a maximally-active split.
Table~\ref{tab:bagel-spec} lists the reconstruction against the published specification, component by component.

\begin{table}[h]\centering\small
\caption{The BAGEL reconstruction against its published specification (their code is unreleased).}\label{tab:bagel-spec}
\begin{tabular}{lll}
\toprule
Component & Published BAGEL & Our reconstruction \\ \midrule
GP mean & zero-mean & zero-mean (matched); ``$+$prior'' adds our calibrated mean \\
...
($+0.109,\dots,+0.114$). BAGEL's embedding covariance cannot \emph{propagate} anchor evidence to a distant
bridge---exactly Lemma~\ref{lem:surface}---so empirically more judgments do not close the gap; it plateaus about
$0.12$ below us.
\end{itemize}
...
features. The conclusion is a clean dichotomy: the graph's advantage is a \emph{distributional} property---a reliable
average over the deep-multi-hop regime, which is exactly what the alignment law describes---and \emph{not} a
per-query-predictable signal. Within the deep multi-hop regime studied, the evidence therefore favors a simple
global graph policy over per-query routing---the latter is neither feasible nor needed here.
...
conflating them---using the graph as a \emph{standalone} ranker, which buries the answer (\S\ref{sec:main})---is
the mistake the field keeps making. (2)~A methodological detail with
