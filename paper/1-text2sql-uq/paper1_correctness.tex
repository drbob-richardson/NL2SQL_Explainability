...
\begin{abstract}
Deciding whether to trust a generated SQL query requires an estimate of whether it is correct, where
correct means the query executes to the same result as a human-written reference query. We study
which signals provide that estimate on hard multi-table text-to-SQL (BIRD), comparing them on equal
footing by how well each ranks correct queries above incorrect ones (\auroc{}), with bootstrapped
confidence intervals. The black-box statistical signals cluster
...
two judge providers. We also ask whether a verifier can be trained. Fine-tuned verifiers, both encoder and generative, reach about
$0.75$--$0.78$ \auroc{} in-distribution but fall to about $0.67$--$0.69$ on unseen schemas, and scaling to
...
A text-to-SQL system turns a natural-language question into a SQL query that runs against a database.
These systems are increasingly used where a wrong query is costly, so a system needs to know not just
how to answer but when it should decline to. Knowing when to decline requires, for each query the
system produces, an estimate of how likely that query is to be correct. We use \emph{correct} in its
standard benchmark sense: the generated query, when executed against the database, returns the same
result as a human-written reference query for the same question. Predicting correctness so a system
can choose to answer or to abstain is the problem of \emph{selective prediction}, and it is the focus
of this paper.

How should such an estimate be produced? Most uncertainty-quantification (UQ) work for text-to-SQL
reads it off statistics of the model's own outputs, without any extra model or labels. The clearest
example is \emph{self-consistency}: draw several queries for the same question and measure how much
they agree, on the premise that a confident model repeats itself \citep{wang2023selfconsistency}.
Related signals include the entropy of the samples' meanings \citep{kuhn2023semantic,farquhar2024semantic}
and how often a particular query structure recurs. We call these \emph{black-box statistical} signals
because they look only at agreement among samples, not at whether a query's logic actually answers the
question. The alternative is \emph{verification}: show a separate model the question, the schema, and
the candidate query, and ask it to judge whether the query is correct (also called LLM-as-judge). We
ask a direct question: on hard text-to-SQL, which of these signals actually predicts correctness, and
by how much?

To answer it we score each signal by its ability to rank correct queries above incorrect ones,
measured as the area under the ROC curve (\auroc{}): the probability that a randomly chosen correct
query is scored above a randomly chosen incorrect one, where $0.5$ is chance and $1.0$ is a perfect
ranking. We attach paired bootstrap confidence intervals to every comparison, so a difference between
two signals can be read as significant or not. Our contribution is this execution-grounded comparison
of the signals available to a practitioner, together with what it implies for deployment.
\begin{enumerate}
...
against the strongest of these, by white-box log-probability (the model's own probability for the
query it generated).
\item Verification moves past the ceiling, with bootstrapped intervals that exclude zero, across two
benchmarks, two generators, and two judge providers. Judges from different providers make different
errors, and an ensemble of them is the best and best-calibrated correctness signal we obtain ($0.82$
\auroc{}, $0.03$ calibration error).
\item A selective-prediction frontier that the self-consistency baseline cannot match.
\item A study of \emph{trained} verifiers, fine-tuned on labeled examples. They do well on databases
seen in training but fail to \emph{transfer} (keep working on databases never seen during training),
which separates an in-domain verifier from a universal one.
\end{enumerate}
...
per question, temperature $0.7$, schema and evidence in the prompt) for a fixed slice of 800 questions
spanning 8 databases. The slice was fixed before any analysis and does not depend on model output: it
takes the questions in benchmark order from each of the 8 databases whose files we held locally,
capped per database and at 800 in total. We execute every generated sample against the real database
and label a query \emph{correct} when its result set matches the reference query's, which is the
standard execution-correctness criterion for these benchmarks. Throughout, the query we score is the
\emph{modal} query, the one a question's samples most often produce. Modal-query execution accuracy is
$0.451$, a hard regime with ample headroom for UQ. For breadth we also use a second generator,
GPT-4.1-mini (accuracy $0.522$), and a second benchmark, Spider's \citep{yu2018spider} multi-table dev
queries ($490$ questions over $20$ databases), generated and executed the same way.
...
token logprobs). All judges see the same input in the same order; the exact prompts are in
Appendix~\ref{app:prompts}.

\textit{Metrics.} We report tie-robust \auroc{} for correctness, with $2{,}000$-sample paired
bootstrap confidence intervals on \auroc{} differences. We also combine signals with a cross-fit
logistic regression (the combiner is fit on one split and scored on a disjoint split, so it never
sees its own test data). For calibration we report expected calibration error (ECE), the average gap
between a score read as a probability and the empirical fraction correct at that score
\citep{guo2017calibration}. For abstention we report a risk--coverage frontier (the selective risk,
or error rate among answered questions, as a function of coverage, the fraction answered), using both
an empirical frontier and a distribution-free (Bonferroni-over-grid, $\delta=0.1$) certificate
\citep{angelopoulos2021ltt}. All model outputs are cached, and the API experiments are inexpensive.
...
elicitation artifact (Appendix~\ref{app:prompts}).
...
\textit{A second benchmark.} The pattern also reproduces on a different benchmark. On Spider's
multi-table dev queries ($490$ questions over $20$ databases; modal accuracy $0.620$), string
self-consistency again sits at the ceiling ($0.618$) and a strong verifier clears it: GPT-4o scores
$0.712$ (paired $\Delta$ over self-consistency $+0.094$, CI $[+0.029, +0.155]$). The weaker GPT-4o-mini
judge is above the ceiling but not significantly so here ($0.669$, $\Delta +0.051$, CI
$[-0.013, +0.114]$), again consistent with the judge needing to be a strong enough reasoner. Across
two benchmarks, two generators, and two providers, then, self-consistency plateaus and a sufficiently
strong verifier moves past it.
...
execution-labeled pairs, and evaluate transfer to unseen databases by leave-one-database-out (LODO):
hold out one database, train on the rest, and score the held-out one, repeating for each database.
We compare an encoder classifier (ModernBERT) and a generative LLM judge (Qwen2.5, LoRA-tuned), with
an un-fine-tuned (zero-shot) baseline. Transfer values are macro-averaged over the held-out databases.
\begin{table}[h]\centering\small
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrr}
\toprule
verifier & in-distribution \auroc{} & transfer (LODO, macro) & transfer (LODO, pooled) \\
\midrule
feature classifier (embeddings, logprob, self-consistency) & $0.768$ & $0.661$ & --- \\
fine-tuned encoder (ModernBERT-base) & $0.750\,[.724,.776]$ & $0.679\,[.656,.706]$ & $0.716\,[.704,.729]$ \\
generative judge, Qwen2.5-1.5B (LoRA) & $0.777\,[.751,.800]$ & $0.670\,[.638,.701]$ & $0.717\,[.705,.730]$ \\
generative judge, Qwen2.5-7B (LoRA) & $0.783\,[.757,.807]$ & $0.685\,[.650,.722]$ & $0.699\,[.686,.713]$ \\
frozen GPT-4o judge (zero-shot) & --- & $0.710\,[.655,.764]$ & $0.770\,[.737,.801]$ \\
\bottomrule
\end{tabular}}
\caption{Trained-verifier transfer with $95\%$ bootstrap CIs. Fine-tuned verifiers plateau near
$0.67$--$0.69$ macro on unseen schemas regardless of architecture or scale; only the frozen reasoning
judge reaches $0.77$ pooled. The judge is scored per question ($n{=}55$--$110$ per held-out database)
and the verifiers per candidate SQL ($n{=}440$--$880$), so its intervals are wider and cross-model
comparisons are read as directional. Trained-verifier numbers are from a re-run
(PyTorch~2.12/Transformers~5.12) that saved per-example scores for the intervals; macro-LODO came in
$0.01$--$0.02$ above the original run across all verifiers, and in-distribution ModernBERT moved from
$0.785$ to $0.750$, consistent with the newer stack.}\label{tab:transfer}
\end{table}

Four findings. First, \emph{fine-tuning works in-distribution}: it lifts the generative judge well
above its zero-shot baseline, and the largest fine-tuned judge reaches the best in-domain number
($0.783\,[.757,.807]$). Second, \emph{it does not transfer}: every fine-tuned verifier, encoder or
generative, lands near $0.67$--$0.69$ macro on unseen schemas---well below its own in-distribution
number---with the worst results on the most domain-specific databases, while the small base model's
near-chance zero-shot transfer shows why a large model is needed. We are deliberate about the
comparison to the frozen judge: its transfer is at or above every fine-tuned verifier's on both
aggregates (macro $0.710\,[.655,.764]$; pooled $0.770\,[.737,.801]$), but the macro intervals overlap
and the judge is scored per question while the verifiers are scored per candidate, so we read the
judge's edge as \emph{directional}. What is \emph{not} within noise is the central claim---fine-tuning
does not lift transfer regardless of architecture or scale: the $7$B judge ($0.685\,[.650,.722]$) is
indistinguishable from the $1.5$B judge ($0.670\,[.638,.701]$) and the encoder ($0.679\,[.656,.706]$),
so the size of the fine-tuned model is not the missing ingredient. Third, \emph{nothing closes the
gap}: scaling to 7B, adding schema diversity, distilling a strong judge's rationales, and
cross-benchmark training all leave transfer at about $0.67$--$0.71$ (Appendix~\ref{app:transfer}); only
the large frozen reasoning judge reaches $0.77$ pooled. The evidence is consistent with the fine-tuned
models relying on schema-specific surface regularities rather than transferable reasoning: an input
ablation of the frozen judge shows it scores $0.692$ from the question and SQL alone, that adding the
schema changes nothing ($0.688$), and that only the evidence helps ($0.724$), so the frozen judge
generalizes by reasoning about the query rather than by reading schema content. Per database
(Figure~\ref{fig:lodo}), the frozen judge leads on average---five of eight held-out schemas---but no
per-database judge-versus-verifier difference is statistically separated (every interval overlaps), so
the per-schema picture is a consistent trend, not a clean sweep.

Fourth, \emph{the in-domain verifier is useful for deployment}. Trained on a database's own schemas,
it predicts correctness at $0.75$--$0.78$ and is cheap at inference. Used as the abstention score, a
...
\caption{Per-database transfer. The frozen GPT-4o judge leads on average (five of eight held-out
schemas), though no per-database judge-versus-verifier difference is statistically separated---its
per-schema intervals are wide because it is scored per question ($n{=}55$--$110$) rather than per
candidate. The pattern is consistent with reasoning transferring across schemas while fine-tuning
captures schema-specific regularities.}
\label{fig:lodo}
\end{figure}
...
For selective prediction we report an empirical risk--coverage frontier (Figure~\ref{fig:rc}). We
split the questions in half: the abstention threshold for each target risk is selected on the
calibration half, and the coverage and risk we report are measured on the disjoint test half, so no
threshold is evaluated on the data that chose it. ``No valid subset'' below means that on the test
half no threshold answers any questions within the target risk.
\begin{table}[h]\centering
\begin{tabular}{lll}
\toprule
...
Natural extensions include an open-weight generator and a third judge provider.
...
\midrule
base fine-tune (8 schemas, LODO) & 0.670 \\
+ schema diversity (28 schemas, LODO) & 0.682 \\
scale generative judge to 7B (LODO) & 0.685 \\
...
\bottomrule
\end{tabular}
\caption{Every attempt to make the fine-tuned verifier transfer lands at $\approx0.65$--$0.71$; only the
frozen judge reaches $0.77$. The base fine-tune and 7B rows are the re-run (with CIs in
Table~\ref{tab:transfer}); the schema-diversity, distillation, and cross-benchmark rows are from the
original run.}\end{table}

\section{Verifier prompts}
\label{app:prompts}
Every verifier sees the same content, presented in this order: the schema, the question, the evidence
string (when available), and the candidate SQL, followed by the elicitation request. Two elicitation
formats are used. The logit format (the OpenAI judges) asks for a one-word verdict and reads
$P(\text{correct})$ from the first-token YES/NO probabilities; the verbalized format (Claude, which
does not expose token logprobs, and the method-matched OpenAI verbal judge) asks for a numeric
probability and divides it by $100$.

\noindent\textbf{Logit (YES/NO) judge.} System: \textit{``You are a strict SQL reviewer. Decide
whether the candidate SQL CORRECTLY answers the question (right tables, columns, conditions,
aggregation, and result). Answer with exactly one word: YES or NO.''} The candidate block ends with
\textit{``Is it correct? Answer YES or NO.''}

\noindent\textbf{Verbalized (0--100) judge.} System: \textit{``You are a strict SQL reviewer. Decide
whether the candidate SQL correctly answers the question. Respond with ONLY an integer from 0 to 100:
the probability (percent) that it is correct. Output only the number.''} The candidate block ends with
\textit{``Probability (0-100) that the SQL is correct:''} The returned integer divided by $100$ is the
