...
generative judge, Qwen2.5-1.5B (LoRA) & 0.766 & 0.659 \\
generative judge, Qwen2.5-7B (LoRA) & 0.798 & 0.662 \\
...
($0.798$). Second, \emph{it does not transfer}: every fine-tuned verifier, encoder or generative,
lands near $0.66$ on unseen schemas, with the worst results on the most domain-specific databases. The frozen large judge is the
...
fine-tuned verifiers. The pattern is consistent with reasoning transferring across schemas while
fine-tuning captures schema-specific regularities.}
\label{fig:lodo}
\end{figure}
