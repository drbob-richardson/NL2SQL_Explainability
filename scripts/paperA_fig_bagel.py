...
def main():
    ...
    R5 = json.load(open(os.path.join(ROOT, "paper", "4-graphrag-A", "bagel_results_n500.json")))
    ...
    b.plot(B, tot, "o-", color="#9467bd", ms=4, lw=1.4, label="total: ours $-$ BAGEL, $N{=}100$")
    b.fill_between(B, covlo, covhi, color="#2ca02c", alpha=0.15)
    b.plot(B, cov, "D-", color="#2ca02c", ms=4, lw=1.4, label="covariance, $N{=}100$")
    cov5 = np.array([R5[str(x)]["gminusbagelprior"][0] for x in B])
    cov5lo = np.array([R5[str(x)]["gminusbagelprior"][1] for x in B]); cov5hi = np.array([R5[str(x)]["gminusbagelprior"][2] for x in B])
    b.fill_between(B, cov5lo, cov5hi, color="#17becf", alpha=0.18)
    b.plot(B, cov5, "D--", color="#0e7c8a", ms=4, lw=1.5, label="covariance, $N{=}500$ (5× deeper)")
    ...
    b.set_title("(B) Covariance-gap flat vs budget, larger at a 5× deeper pool", fontsize=8.7)
    b.legend(fontsize=7.0, loc="upper right", frameon=False); b.grid(alpha=0.25); b.set_ylim(-0.02, 0.30)
